        ...
        <ResponsiveContainer width={"100%"} minHeight={300}>
          <PieChart>
            <Pie
              ...
              dataKey="Count"
            >
              { this.props.data.map((entry, index) => <Cell entry={entry} key={`cell-${index}`} fill={COLORS[index % COLORS.length]}/>)}
            </Pie>
            <Legend
              formatter={(value, entry, index) => (entry.payload.payload.payload.Host.replace('(',''))} // forgive this it is a deeply nested object
            />
