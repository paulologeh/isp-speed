...
class App extends Component {
  state = {
    ...
    summary: [
      {
        meta: "Change Since Yesterday: 0 %",
        header: "Average Download Speed",
        description: "0 Mbps",
      },
      {
        meta: "Change Since Yesterday: 0 %",
        header: "Average Upload Speed",
        description: "0 Mbps",
      },
      {
        meta: "Tests Today: 0",
        header: "Total Number of Tests",
        description: "0",
      },
  ...
  async callAPI() {
    ...
    summary = getSummary(
      summary,
      data.recordset,
      this.state.minimumDownload,
      this.state.minimumUpload,
      data.recordset
    );
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (
      prevState.minimumDownload !== this.state.minimumDownload ||
      prevState.minimumUpload !== this.state.minimumUpload
    ) {
      let summary = [...this.state.summary];
  ...
  handleMinDownload = (event) =>
    this.setState({ minimumDownload: event.target.value });
  handleMinUpload = (event) =>
    this.setState({ minimumUpload: event.target.value });
            ...
            <Grid.Row>
              ...
              <Grid columns={Mobile() ? 1 : "equal"}>
                <Grid.Column>
                  <Segment>
                    <Header as="h3" textAlign="center">
                      Average Cummulative Speed By Provider
                    </Header>
